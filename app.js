...
function load(){ const initial={transactions:[],goal:{name:'貯金目標',target:0,saved:0,deadline:''},budget:0,assets:{total:0,asOf:''},categories:{expense:[],income:[]}}; try { const saved=JSON.parse(localStorage.getItem(storeKey)); return saved ? {...initial,...saved,goal:{...initial.goal,...saved.goal},assets:{...initial.assets,...saved.assets},categories:{...initial.categories,...saved.categories}} : initial; } catch { return initial; } }
...
function render(){
  ...
  const assets=data.assets||{}; $('assetValue').textContent=assets.total?yen(assets.total):'未入力'; $('assetNote').textContent=assets.asOf?`${assets.asOf.replaceAll('-','/')} 時点`:'収支とは別に管理します';
  ...
  const {name,target,saved,deadline}=data.goal; $('goalTitle').textContent=name||'貯金目標'; $('savedAmount').textContent=yen(saved||0); $('goalAmount').textContent=target?`目標 ${yen(target)}`:'目標を設定'; const percent=target?Math.min(100,(saved/target)*100):0; $('goalProgress').style.width=`${percent}%`; const left=Math.max(0,target-saved); const days=deadline?Math.ceil((new Date(`${deadline}T00:00:00`)-new Date(`${dateValue()}T00:00:00`))/86400000):null; $('goalNote').textContent=target?(saved>=target?'目標達成です！おめでとうございます。':days!==null?(days<0?`期限を過ぎています · あと ${yen(left)}`:`期限まであと ${days}日 · あと ${yen(left)}`):`目標まであと ${yen(left)}`):'まずは目標を設定しましょう';
...
$('editGoal').onclick=()=>$('goalsButton').click(); $('goalsButton').onclick=()=>{const g=data.goal;$('goalNameInput').value=g.name==='貯金目標'?'':g.name;$('goalTargetInput').value=g.target||'';$('goalSavedInput').value=g.saved||'';$('goalDeadlineInput').value=g.deadline||'';$('goalDialog').showModal();};
$('goalForm').addEventListener('submit',e=>{e.preventDefault();data.goal={name:$('goalNameInput').value.trim()||'貯金目標',target:Number($('goalTargetInput').value),saved:Number($('goalSavedInput').value),deadline:$('goalDeadlineInput').value};persist();$('goalDialog').close();render();});
$('editBudget').onclick=$('editAssets').onclick=()=>{$('monthlyBudgetInput').value=data.budget||'';$('assetTotalInput').value=data.assets?.total||'';$('assetAsOfInput').value=data.assets?.asOf||dateValue();$('moneyDialog').showModal();};
$('moneyForm').addEventListener('submit',e=>{e.preventDefault();data.budget=Number($('monthlyBudgetInput').value)||0;data.assets={total:Number($('assetTotalInput').value)||0,asOf:$('assetAsOfInput').value};persist();$('moneyDialog').close();render();});
